    ...
    for i in arr:
        ...
        if gui:
            ...
            if rec_vid:
                ...
                file_path = kwargs.get('rec_file_path', "data/best"+str(num)+".mp4")
        ...
        # # create ramp
        # ramp_half_extents = [30, 6, 0.5]
        # ramp_visual_shape_id = p.createVisualShape(p.GEOM_BOX, halfExtents=ramp_half_extents, rgbaColor=[0.5, 0.5, 0.5, 1])
        # ramp_collision_shape_id = p.createCollisionShape(p.GEOM_BOX, halfExtents=ramp_half_extents)
        # ramp_body_id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=ramp_collision_shape_id, baseVisualShapeIndex=ramp_visual_shape_id,basePosition=[0, 0, 0])

        # # set ramp position and orientation
        # ramp_position = [-0.55, 0, -0.55]  # adjust as needed
        # ramp_orientation = p.getQuaternionFromEuler([0, -15 * math.pi / 180, 0])  # 15-degree ramp
        # p.resetBasePositionAndOrientation(ramp_body_id, ramp_position, ramp_orientation)

        # friction = 0.9
        # restitution = 1.0
        # p.changeDynamics(ramp_body_id, -1, lateralFriction=friction, restitution=restitution)
        ...
        while step <= 10000:
            ...
            if gui:
                ...
                if s.num_components == 1:
                    p.resetDebugVisualizerCamera(cameraDistance=0.7, cameraYaw=-90, cameraPitch=-60,
            ...
            if (step > 200) and (nxt_pos[1] > 5 or nxt_pos[1] < -5 or nxt_pos[0] < -0.5):
                violation = True
                break

            step = step + 1
            #while True and step > 5:
            #    pass
